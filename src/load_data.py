...
def load_data():
    ...
    for idx, cls in enumerate(classes):
        ...
        for fname in os.listdir(cls_dir):
            if fname.endswith('.png') or fname.endswith('.jpg'):
                ...
                images.append(img)
                labels.append(idx)
    images = np.expand_dims(np.array(images), -1)  # (N, 28, 28, 1)
    ...
    def augment(image):
        ...
        # Use Keras layers for translation and rotation (work with tensors)
        image = tf.keras.layers.RandomTranslation(0.1, 0.1, fill_mode='nearest')(tf.expand_dims(image, 0))[0]
    ...
    # Append augmented images to the original training data
    X_train = np.concatenate([X_train, np.array(X_train_aug)], axis=0)
    y_train = np.concatenate([y_train, y_train], axis=0)  # Duplicate labels for augmented images
